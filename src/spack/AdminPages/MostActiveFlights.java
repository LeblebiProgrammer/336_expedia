...
//			out.println("<input name=\"click\" type=\"submit\" value=\"Book for User\" /> ");
  ...
  protected void finishHtml(java.io.PrintWriter out) {
    ...
    try {
			...
			stmt=con.prepareStatement("create temporary table t select f.FlightID, count(distinct r.ReservationNumber) as ticketsSold from Flights f join Reservation r using (FlightID) group by f.FlightID select t.FlightID from t where t.FlightID = (select max(t.ticketsSold) from t)");  
